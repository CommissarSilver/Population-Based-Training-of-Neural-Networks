...
class ActorCriticNetwork(tf.keras.Model):
    def __init__(self, observation_dims, output_dims, name='ActorCritic', checkpoint_directory='\Agent-Models'):
        ...
        self.dense_layer_2 = tf.keras.layers.Dense(units=512, activation='relu', name='Dense_Layer_2',
                                                   dtype=tf.float32)
        self.dense_layer_3 = tf.keras.layers.Dense(units=256, activation='relu', name='Dense_Layer_3',
                                                   dtype=tf.float32)
        self.dense_layer_4 = tf.keras.layers.Dense(units=32, activation='relu', name='Dense_Layer_4',
                                                   dtype=tf.float32)
        self.dense_layer_5 = tf.keras.layers.Dense(units=16, activation='relu', name='Dense_Layer_5',
                                                   dtype=tf.float32)
    ...
    def call(self, state):
        ...
        y = self.dense_layer_5(x)  # use dense layer 3 as an extra layer for critic
